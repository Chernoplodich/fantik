"""LikesRepository: лайки пользователя. Счётчик fanfics.likes_count правит use case."""
...
from app.infrastructure.db.models.like import Like as LikeModel


class LikesRepository(ILikesRepository):
    def __init__(self, session: AsyncSession) -> None:
        self._s = session

    async def exists(self, user_id: UserId, fic_id: FanficId) -> bool:
        stmt = select(LikeModel.user_id).where(
            LikeModel.user_id == int(user_id),
            LikeModel.fic_id == int(fic_id),
        )
    ...
    async def list_by_user(self, user_id: UserId, limit: int, offset: int) -> list[FanficId]:
        stmt = (
            select(LikeModel.fic_id)
            .where(LikeModel.user_id == int(user_id))
