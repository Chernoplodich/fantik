...
- [x] **Этап 8** — UX-итерация (фандомы + поиск + меню): миграция 0010 включает `pg_trgm` + `GIN(name gin_trgm_ops)`, переводит существующие записи `movies → films`, точечно правит ошибочные `series` (stranger-things/sherlock-bbc/supernatural/doctor-who), переименовывает кривой slug `genshin-anime → demon-slayer` и заливает ~155 новых фандомов по 11 категориям ficbook (anime/books/films/series/cartoons/comics/games/musicals/rpf/originals/other); миграция 0011 добавляет таблицу `fandom_proposals` с ENUM `fandom_proposal_status` + partial-unique `(requested_by, LOWER(name)) WHERE status='pending'` для анти-дубля. Доменный агрегат `FandomProposal` с state-machine `pending → approved | rejected` + события `Submitted/Approved/Rejected`; use cases `SubmitFandomProposalUseCase` (анти-дубль через ConflictError → `created=False`) / `ApproveFandomProposalUseCase` (внутри транзакции вызывает `CreateFandomUseCase` и помечает proposal с `created_fandom_id`) / `RejectFandomProposalUseCase` / `ListPendingFandomProposalsUseCase`; `FandomProposalNotifier` шлёт автору уведомления через aiogram (TelegramAPIError → silent skip). `IReferenceReader` расширен методами `search_fandoms(query, limit, category)` (PG ILIKE `name` + EXISTS unnest(aliases) ILIKE, prefix-match priority) и `list_fandoms_by_category`. Общий двухступенчатый пикер фандомов `keyboards/fandom_picker.py` (single/multi mode) используется и в мастере создания фика, и в фильтре поиска: шаг 1 — категории (8-11 кнопок 2 в ряд), шаг 2 — фандомы внутри категории (10/стр), плюс `🔍 Найти по названию` (FSM ввод текста) и `➕ Предложить свой` (только в create). В корне фильтров `keyboards/search_filters.py` появилась кнопка `✏️ Запрос (—)` (свободный q-text через FSM `entering_query`) + детальный help-текст. Главное меню `keyboards/main_menu.py` — динамический `b.adjust(...)` ставит кнопки `🛡 Модерация` и `⚙️ Админ` в одну строку для ADMIN. Раздел `/admin → 📋 Заявки на фандом` (роутер `admin_proposals.py` + клавиатуры `admin_fandom_proposals.py`) с FSM `FandomProposalReviewFlow.waiting_reject_reason` для отказа с причиной. `_ALLOWED_CATEGORIES` расширен до 11 кодов (legacy `movies` сохранён). `admin_fandoms.py` использует общую таблицу `fandom_categories.CATEGORIES` для лейблов. Meili-индекс получил `fandom_category` в `filterableAttributes` (для будущего фильтра по разделу). 32 новых unit-теста (домен FandomProposal, use cases proposals, main_menu_layout, fandom_picker_kb, search_query_flow, search_filters_root_kb) + 2 интеграционных модуля (search_fandoms_repo, fandom_proposals_repo); полный unit-suite 347/347 зелёный.
