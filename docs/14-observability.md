...
В процессе `bot` поднят aiohttp-эндпоинт `/metrics` (порт 8080) от prometheus-client:
...
В worker-процессах — отдельный порт 8081 (`metrics_port` per role).
...
- `users_total` — gauge, ежеминутно из БД.
- `active_users_24h` — gauge, из materialized view.
- `fics_approved_total` — gauge.
- `search_queries_total` — counter.
- `search_cache_hit_ratio` — gauge (производное).
...
Пресеты (JSON-панели, можно сохранить в `docker/grafana/dashboards/`):
