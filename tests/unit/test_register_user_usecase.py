...
class TestRegisterUser:
    ...
    async def test_existing_user_with_utm_records_start_for_funnel(
        self, clock: FrozenClock
    ) -> None:
        """Регрессия: воронка по ссылке должна расти на повторных переходах.

        До фикса tracking-events писались ТОЛЬКО для is_new=true, поэтому
        существующие юзеры, пришедшие по новой UTM, не учитывались
        в `transitions/unique_users` фуннела. Теперь `start` пишется для
        каждого /start с UTM, независимо от того, новый юзер или нет.
        """
        users, tracking, uow = FakeUsers(), FakeTracking(), FakeUow()
        # Юзер уже существует, без UTM (зашёл когда-то раньше).
        await users.save(
            User.register(
                tg_id=777,
                username="alice",
        ...
        tracking.add_code("vkpaid001", cid=42)
        uc = RegisterUserUseCase(uow, users, tracking, clock)
        ...
        assert result.is_new is False
        # Только `start` (для воронки), без `register` (юзер уже был).
        types = [e.event_type.value for e in tracking.events]
        assert types == ["start"]
        assert tracking.events[0].code_id == TrackingCodeId(42)

    async def test_existing_user_with_utm_attaches_first_touch(
        self, clock: FrozenClock
    ) -> None:
        """Late-first-touch: если у юзера ещё нет UTM, и он пришёл по новой —
        присваиваем (это first для него). Без этого админ-выгрузка по UTM
        даёт пустой список даже когда юзеры реально переходят по ссылке."""
        users, tracking, uow = FakeUsers(), FakeTracking(), FakeUow()
        await users.save(
            User.register(
                tg_id=777,
    ...
    async def test_existing_user_with_utm_does_not_overwrite_first_touch(
        self, clock: FrozenClock
    ) -> None:
        """Жёсткий контракт: first-touch UTM никогда не перезаписывается.

        Юзер уже привязан к коду 1. Если он позже пришёл по коду 2 —
        users.utm_source_code_id остаётся 1. (Воронки получат `start` для
        обеих ссылок, но first-touch принадлежит первой.)
        """
        users, tracking, uow = FakeUsers(), FakeTracking(), FakeUow()
        tracking.add_code("first0001", cid=1)
        tracking.add_code("second002", cid=2)
        await users.save(
            User.register(
                tg_id=777,
                username="alice",
        ...
        assert result.user.utm_source_code_id == TrackingCodeId(1)
        # `start` пишется (event для воронки code=2), но user остался first-touch=1.
        assert tracking.events[0].code_id == TrackingCodeId(2)
